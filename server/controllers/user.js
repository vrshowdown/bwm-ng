...
function activationEmail(req, res, email) {
    ...
    User.findOne({ email }, function (err, foundUser) {
        ...
        if (foundUser.id) {
            ...
            sendSmtpEmail.sender = { email: 'jibreelutley@jmu3d.com', name: 'BookWithMe' };
            sendSmtpEmail.to = [{ email: foundUser.email, name: foundUser.username }];
            sendSmtpEmail.subject = 'BookWithMe Account Activation';
            sendSmtpEmail.textContent = 'Hello ' + foundUser.username + ', You recently Registered to a new account. click this link ' + address + token;
            sendSmtpEmail.htmlContent = 'Hello ' + foundUser.username + '</strong>,<br><br>, You recently registered to a new account. Please click this link ' + '<a href ="' + address + token + '">Activate Account</a>';

            brevoApiInstance.sendTransacEmail(sendSmtpEmail)
                .then((data) => {
                    //Celebrate - email sent successfully
...
exports.forgotPassword = function (req, res) {
    ...
    User.findOne({ email }, function (err, existingUser) {
        ...
        let sendSmtpEmail = new SibApiV3Sdk.SendSmtpEmail();
        sendSmtpEmail.sender = { email: 'jibreelutley@jmu3d.com', name: 'BookWithMe' };
        sendSmtpEmail.to = [{ email: existingUser.email, name: existingUser.username }];
        sendSmtpEmail.subject = 'BookWithMe Password Reset';
        sendSmtpEmail.textContent = 'Hello ' + existingUser.username + ', You recently requested reset password. click this link ' + address + token;
        sendSmtpEmail.htmlContent = 'Hello ' + existingUser.username + '</strong>,<br><br>, You recently requested to reset password. Please click this link ' + '<a href ="' + address + token + '">Reset Password</a>';
